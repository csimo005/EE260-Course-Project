...
def load_data(path, file_dict):
	...
	lab_tr = np.zeros([num_tr, num_cl], dtype=np.uint8)
	img_te = np.zeros([num_te, img_h*img_w])
	lab_te = np.zeros([num_te, num_cl], dtype=np.uint8)
	...
	for data in ['train', 'test']:
		...
		if data == 'train':
			for i in range(num_tr):
				...
				lab_tr[i][np.frombuffer(buf_lab, dtype=np.uint8)] = 1
			...
			for i in range(num_te):
				...
				lab_te[i][np.frombuffer(buf_lab, dtype=np.uint8)] = 1
